...
Given a string containing only three types of characters: '(', ')' and '*', write a function to check whether this string is valid. We define the validity of a string by these rules:
	<p>1. Any left parenthesis '(' must have a corresponding right parenthesis ')'.
	<p>2. Any right parenthesis ')' must have a corresponding left parenthesis '('.
	<p>3. Left parenthesis '(' must go before the corresponding right parenthesis ')'.
	<p>4.'*' could be treated as a single right parenthesis ')' or a single left parenthesis '(' or an empty string.
	<p>5. An empty string is also valid.
	...
	/*public boolean isValid(String s) {
		...
		for(int i=0;i<arr.length;i++){
			...
			if(arr[i]=='(' || arr[i]==')' || arr[i]=='*') {
				...
				System.out.println("Invalid input charcter:" + arr[i]);
				return false;
			}
		}
		
		
		return ((parenthese==0) || (parenthese+asterisk)==0);
...
Time Complexity: O(N * 3^{N}) where NN is the length of the string. For each asterisk we try 3 different values. Thus, we could be checking the validity of up to 3^N 
